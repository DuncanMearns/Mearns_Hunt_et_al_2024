...
if __name__ == "__main__":
    ...
    eye_analysis_dir = EXPT_DIR.joinpath("analysis", "eye_analysis", SPECIES)

    output_dir = EXPT_DIR.joinpath("tracking", "artemia", "sequences", SPECIES)
    ...
    df = pd.read_csv(eye_analysis_dir.joinpath("events.csv"))
    df = df[~df["boundary"]]
    ...
    for video_id, video_events in df.groupby("video_id"):
        ...
        start_frames = video_events["start"].values
        end_frames = video_events["end"].values
